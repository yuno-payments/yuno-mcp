import z from "zod";
import { cardDataSchema } from "../shared/types";
...
export const paymentMethodEnrollSchema = z.object({
  account_id: z.string().min(36).max(64).optional().describe("The unique identifier of the account. You find this information on Yuno's Dashboard (MAX 64; MIN 36)."),
  ...
  }).partial().optional().describe("Provider data for token migration, only if agreed with Yuno."),
  ...
  }).optional().describe("Indicates whether to verify the payment with a verify transaction or not. null if not specified."),
})
